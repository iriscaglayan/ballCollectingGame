...
public class Archer : CogsAgent


// Strategy: We will be giving our robot a high reward for bringing back balls to its homebase,
// a moderate reward for hitting an enemy or bringing back multiple balls to its homebase,
// a very small reward for firing a laser, and a small punishment for being frozen.
// Using these values, we will run training on our robot to allow it to develop the best strategy
// to compete against other robots.

{
    protected float shootTime = 120f;
    ...
    private void AgentAhead()
    {
        ...
        if (Physics.Raycast(transform.position, transform.forward, out hit, 20))
        {
            ...
            if (hit.transform.CompareTag("Player") & ((shootTime - 3) >= timer.GetComponent<Timer>().GetTimeRemaning()))
            {
                SetLaser(true);
                //UnityEngine.Debug.Log("LaserOn, " + timer.GetComponent<Timer>().GetTimeRemaning());
                shootTime = timer.GetComponent<Timer>().GetTimeRemaning();
            }
            else if ((shootTime - .35)! >= timer.GetComponent<Timer>().GetTimeRemaning())
            {
                SetLaser(false);
                //UnityEngine.Debug.Log("LaserOff, " + timer.GetComponent<Timer>().GetTimeRemaning());
            }
        }
    ...
    protected override void OnTriggerEnter(Collider collision)
    {
        ...
        if (collision.gameObject.CompareTag("HomeBase") && collision.gameObject.GetComponent<HomeBase>().team == GetTeam())
        {
            ...
            {
                //adds reward once per target dropped off at base
                AddReward(0.8f);
            }
        }
    ...
    protected override void OnCollisionEnter(Collision collision)
    {
        ...
        //called when picking up a target
    ...
    private void AssignBasicRewards()
    {
        ...
        //added when our agent is frozen
        rewardDict.Add("frozen", -0.05f);
        rewardDict.Add("shooting-laser", 0.01f);
        rewardDict.Add("hit-enemy", 0.5f);
        //added once per target carried when hit by a lazer
        rewardDict.Add("dropped-one-target", -1f);
        //added once whenever hit by a laser and carrying any amount of targets
